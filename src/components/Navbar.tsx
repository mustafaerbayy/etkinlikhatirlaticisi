    ...
    <nav className="sticky top-0 z-50 border-b bg-card/80 backdrop-blur-md">
      <div className="container mx-auto flex items-center justify-between px-4 py-3">
        ...
        <div className="hidden items-center gap-6 md:flex">
          ...
          {user && (
            <Link to="/profil" className="text-sm font-medium text-foreground/80 hover:text-foreground transition-colors">
              Hatırlatıcılar
            </Link>
          )}
          {isAdmin && (
            <Link to="/yonetim" className="text-sm font-medium text-foreground/80 hover:text-foreground transition-colors">
              Yönetim
            </Link>
          )}
          {user ? (
